...
export default function WhySpf({data,title}:WhySpfProps) {
  return (
    <Box sx={{width:'95%',margin:'90px auto'}}>
        <Typography style={{color:'black',fontSize:'28px',fontWeight:'400',textAlign:'start',marginBottom:'40px'}}>
           <Avatar src={Logo} style={{width:'70px',height:'70px',marginRight:'10px',fontSize:'28px'}} shape="square" /> Why {title} </Typography>
           {
            data.map((item,_index)=>(
              <>
               <Typography style={{display:'flex',fontWeight:'400',textAlign:'start',marginLeft:'70px',lineHeight:'40px',fontSize:'18px',marginTop:'5px', alignItems:'center'}}>
                ...
                <Typography style={{textAlign:'start',marginLeft:'110px',lineHeight:'40px',fontSize:'16px'}}>{item.text}</Typography>
